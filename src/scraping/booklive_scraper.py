"""
BookLive スクレイパー
ebookjapanの成功パターンを適用したコンテナベース抽出
"""
import asyncio
import re
import logging
...
class BookLiveScraper(RequestsScraper):
    """BookLive 高度解析版スクレイパー"""
    
    SITE_NAME = "booklive"
    BASE_URL = "https://booklive.jp"
    SEARCH_URL = "https://booklive.jp/search/keyword"
    ...
    def _get_search_strategies(self, title: str, n_code: str = "") -> List[Dict[str, Any]]:
        ...
        # タイトルのバリエーション生成
        title_variants = self._create_booklive_title_variants(title)
        ...
        for i, variant in enumerate(title_variants):
            strategies.append({
                ...
                'params': {
                    'keyword': variant  # BookLiveでは'keyword'パラメータを使用
                }
            })
        
        return strategies
    
    def _create_booklive_title_variants(self, title: str) -> List[str]:
        """BookLive用タイトルバリエーション生成"""
        ...
        # 巻数表記のバリエーション
        volume_variants = self._create_volume_variants_booklive(title)
    ...
    def _find_book_containers(self, soup: BeautifulSoup) -> List[Tag]:
        ...
        for selector in container_selectors:
            try:
                if ':has(' in selector:
                    # :has() 疑似クラスは手動実装
                    link_elements = soup.select('a[href*="/product/"]')
        ...
        for container in all_containers:
            ...
            if container_id not in seen:
                ...
                book_links = [link for link in links if '/product/' in link.get('href', '')]
    ...
    async def _verify_url(self, url: str, expected_title: str) -> bool:
        ...
        try:
            ...
            # BookLiveの書籍URLパターンチェック
            valid_patterns = ['/product/', '/book/', '/title/']

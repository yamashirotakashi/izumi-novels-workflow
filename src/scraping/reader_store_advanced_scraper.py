"""
Reader Store 高度ブラウザ自動化スクレイパー（リファクタリング版）
...
class ReaderStoreAdvancedScraper(BaseBrowserManager):
    """Reader Store 高度ブラウザ自動化スクレイパー（リファクタリング版）"""
    ...
    async def search_book(self, book_title: str, n_code: str = "") -> Optional[str]:
        ...
        try:
            logger.info(f"Reader Store検索開始: {book_title} ({n_code})")
            ...
            for i, variant in enumerate(title_variants):
                ...
                try:
                    ...
                    # 結果解析
                    result = await self._extract_reader_store_search_results(variant)
                    if result:
                        logger.info(f"Reader Store検索成功: {book_title} -> {result}")
                        return result
            ...
            logger.warning(f"Reader Store検索失敗: {book_title}")
            return None
            
        except Exception as e:
            logger.error(f"Reader Store検索エラー: {book_title} - {str(e)}")
            return None
    
    async def _extract_reader_store_search_results(self, query: str) -> Optional[str]:
        """Reader Store検索結果の抽出（リファクタリング版）"""
        try:
            # ページソース取得（共通基盤使用）
            soup = self.get_page_soup()
            
            # Reader Store特有のセレクタとURLパターン
            reader_store_selectors = [
                'div[class*="product"]',
                'div[class*="book"]',
                'div[class*="item"]',
                'div[class*="result"]',
            ...
            reader_store_url_patterns = ['/storeProduct/', '/item/', '/product/', 'ebookstore.sony.jp']
            
            # 書籍コンテナ発見（共通基盤使用）
            result_containers = self.find_book_containers(
                soup, 
                custom_selectors=reader_store_selectors,
                url_patterns=reader_store_url_patterns
            )
            
            if not result_containers:
                logger.debug(f"Reader Store検索結果コンテナが見つかりません: {query}")
                return None
            
            logger.debug(f"Reader Store検索結果コンテナ数: {len(result_containers)}")
            ...
            for i, container in enumerate(result_containers[:15]):
                try:
                    ...
                    # Reader Store特有のボーナス
                    if 'ebookstore.sony.jp' in url:
                        score += 0.1
                    if '/storeProduct/' in url:  # Sony詳細ページ
                        score += 0.15
                    if len(title) > 5:
                        score += 0.05
                    
                    logger.debug(f"Reader Store書籍候補 {i+1}: '{title[:50]}...' -> スコア {score:.3f}")
                    ...
                    logger.warning(f"Reader Store書籍情報抽出エラー: {str(e)}")
                    continue
            
            if best_match:
                logger.info(f"Reader Store最適マッチ発見 (スコア: {best_score:.3f}): {best_match}")
                return best_match
            else:
                logger.warning(f"Reader Store適切なマッチが見つかりませんでした: {query}")
                return None
            
        except Exception as e:
            logger.error(f"Reader Store検索結果抽出エラー: {str(e)}")
            return None
    
    def create_title_variants(self, title: str) -> List[str]:
        """Reader Store用タイトルバリエーション生成（リファクタリング版）"""
        variants = set()
        ...
        # シリーズ名のみ
        series_only = self._extract_reader_store_series_name(title)
    ...
    def extract_book_info(self, container) -> Optional[Dict[str, str]]:
        """Reader Store書籍情報抽出（リファクタリング版）"""
        try:
            info = {}
            
            # URL抽出（優先度順）
            url_selectors = [
                'a[href*="/storeProduct/"]',   # 最優先（Sony詳細ページ）
                'a[href*="/item/"]',           # 次優先
                'a[href*="/product/"]',        # 補助
                'a[href*="ebookstore.sony.jp"]',  # Sony サイト内
                'a[href]'                      # フォールバック
            ]
            ...
            # タイトル抽出
            title = self._extract_reader_store_title(container, url_element)
            ...
            logger.debug(f"Reader Store書籍情報抽出エラー: {str(e)}")
            return None
    
    def _extract_reader_store_series_name(self, title: str) -> str:
        """Reader Storeシリーズ名抽出"""
        patterns = [
            r'[①-⑳]',
            r'第\d+巻',
    ...
    def _extract_reader_store_title(self, container, url_element=None) -> Optional[str]:
        """Reader Storeタイトル抽出"""
        ...
        for text_node in container.find_all(string=True):
            ...
            if len(text) > 5 and not text.isdigit() and '円' not in text:

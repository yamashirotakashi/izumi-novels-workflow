...
"""
honto サイト構造解析ツール
ebookjapan・BookLiveで成功したアプローチを適用
"""
import asyncio
import requests
import json
...
def analyze_honto_structure():
    ...
    test_queries = [
        {
            'query': 'ソードアート・オンライン',
            'params': {'k': 'ソードアート・オンライン', 'tb': 'ebook'}
        },
        {
            'query': 'SAO',
            'params': {'k': 'SAO', 'tb': 'ebook'}
        },
        {
            'query': '転生したらスライムだった件',
            'params': {'k': '転生したらスライムだった件', 'tb': 'ebook'}
        },
        {
            'query': 'リゼロ',
            'params': {'k': 'リゼロ', 'tb': 'ebook'}
        },
        {
            'query': 'この素晴らしい世界に祝福を',
            'params': {'k': 'この素晴らしい世界に祝福を', 'tb': 'ebook'}
        },
        {
            'query': 'パラレイドデイズ',
            'params': {'k': 'パラレイドデイズ', 'tb': 'ebook'}
        }
    ]
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # 検索ボックスの確認
            search_inputs = soup.find_all(['input[type="search"]', 'input[name*="search"]', 'input[name*="k"]'])
    ...
    for i, test_query in enumerate(test_queries, 1):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                # 書籍リンクのパターン検索（honto特有も含む）
                book_link_patterns = [
                    'a[href*="/netstore/pd-book/"]',  # honto特有パターン
                    'a[href*="/netstore/pd-ebook/"]',  # honto電子書籍
                    'a[href*="/book/"]',
                    'a[href*="/ebook/"]',
                    'a[href*="/detail/"]',
                    'a[href*="/product/"]',
                ...
                # 書籍っぽいパターンの検索
                book_title_patterns = soup.find_all(string=lambda text: 
                    text and (
    ...
    # 結果をJSONファイルに保存
    output_file = f"logs/honto_analysis_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def main():
    """メイン処理"""
    try:
        results = analyze_honto_structure()

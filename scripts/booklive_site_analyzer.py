...
def analyze_booklive_structure():
    """BookLiveの基本構造を解析"""
    ...
    for i, test_query in enumerate(test_queries, 1):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                # 書籍リンクのパターン検索（BookLive特有も含む）
                book_link_patterns = [
                    'a[href*="/product/"]',  # BookLive特有パターン
                    'a[href*="/book/"]',
                    'a[href*="/title/"]',
                    'a[href*="/detail/"]',
                    'a[href*="/item/"]',
    ...
    # 結果をJSONファイルに保存
    output_file = f"logs/booklive_analysis_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def main():
    """メイン処理"""
    try:
        results = analyze_booklive_structure()

...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('logs/booklive_test.log')
    ]
)
logger = logging.getLogger(__name__)


async def test_booklive():
    """BookLive スクレイパーのテスト実行"""
    ...
    print("=== BookLive スクレイパーテスト開始 ===")
    print(f"テスト開始時刻: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"テスト対象: {len(test_books)}冊")
    print("技術: ebookjapan成功パターンを適用したコンテナベース抽出")
    print()
    
    # スクレイパー初期化
    scraper = BookLiveScraper(
        timeout=15,
        max_retries=2
    )
    ...
    # 結果をJSONファイルに保存
    output_file = f"logs/booklive_test_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    print(f"  BookLive（今回）: 成功率 {success_rate:.1f}% ({len(successful_results)}/{len(results)})")
    
    if success_rate >= 100.0:
        print(f"  🎉 完璧な結果！")
    elif success_rate >= 66.7:
        print(f"  🎉 ebookjapanレベルの高性能！")
